...
def aug_images(img_raw, img_size=(299, 299), crop=False, flip=True, r90_270=True, r180=True):
    (w, h) = img_raw.size
    if h <= w:
        b = (w-h)//2
    ...
    if crop:
        ...
        imgs = [img_raw.resize(img_size, Image.LANCZOS)]
...
def aug_predict(models, img0):
    ...
    for model in models:
        ...
        imgs = aug_images(img, (w, h), **config)
